jQuery(document).ready(function($) {
	...
	if(page_number_next <= page_number_max) {
		...
		$content.after('<div id="byscripts_ajax_posts_loader_trigger" class="more">{loadmore}</div>'.replace('{loadmore}', byscripts_ajax_posts_loader.load_more_str));
	 ...
	 */
	if($("#byscripts_ajax_posts_loader_trigger").length) {
		$loaderTrigger.click(function() {
			...
			if(page_number_next <= page_number_max) {
				...
				$(this).html(byscripts_ajax_posts_loader.loading_str);
				...
				$.ajax({
					...
					$content.append($(data).find(byscripts_ajax_posts_loader.content_css_selector).remove(byscripts_ajax_posts_loader.pagination_css_selector).html());
					$(byscripts_ajax_posts_loader.pagination_css_selector).remove();
	
					if(page_number_next <= page_number_max) {
						$loaderTrigger.html(byscripts_ajax_posts_loader.load_more_str);
					} else if(byscripts_ajax_posts_loader.remove_link_after_last_result) {
						$loaderTrigger.remove();
					} else {
						$loaderTrigger.html(byscripts_ajax_posts_loader.no_more_str);
					}
					$(".blog-main .items-wrap").packery("destroy");
					initBlogMainPage();
				}).fail(function() {
					$loaderTrigger.html(byscripts_ajax_posts_loader.error_str);
				});
			}
