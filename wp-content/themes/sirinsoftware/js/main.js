...
(function($) {
	...
	function initPageParams(){
		...
		$(".footer .btn-page-begin").each(function(){
			...
			$(window).on("load scroll resize", function(){			
				...
				var p1 = $(".footer .rightcol").offset().top + $(".footer .rightcol").outerHeight();
				...
				if ( window.innerWidth >= 600 ) {					
					...
					if ((p2 - p1 + $(".footer .copyright").height() + 16) < 0) {
						bottom = 0;	
					} else {
						bottom = p2 - p1 + $(".footer .copyright").height() + 16 + "px";		
					}
					$btn.css({
						"left": left,
						"bottom": bottom, 
		...
		(function(){
			...
			$(document).ready(function(){
				if(!/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent) 
					...
					$(".video-main").delay(2000).queue(function() {
						if($('#playerVideo1').length){
							$(".video-main").addClass("hide-bg");
						}
				    });
				}
			});
			...
			/*
			$(".video-main").delay(1000).queue(function() {
		...
		(function(){
			...
			function setSlider(){				
				var minH;				
				if ( window.innerHeight > window.innerWidth && window.innerHeight >= 568 && window.innerWidth <= 360) {	
					minH = window.innerHeight - $(".toolbar").outerHeight() + "px";						
				} else {
					minH = "0";
				}				
				$(".slider-main .slider ul").trigger("destroy");				
				$(".slider-main .slider ul li").css("min-height", minH).show();								
				$(".slider-main .slider ul").each(function(){
					...
					$(this).carouFredSel({
						...
						prev: $(this).closest(".slider-main").find(".btn-slider-prev"),
						next: $(this).closest(".slider-main").find(".btn-slider-next") 
					});
				});
			}
		})();
		...
		(function(){
			...
			$(window).on("load resize", function(){
				...
				$(".divisions .items-wrap").css('display','block');
			}).trigger("load");	
		})();
		...
		*/
			...
			function setActiveTab(){
				$(".portfolio-start-tabs a").on("click", function(e){
					...
					if (window.history.pushState) {
					   ...
					   window.history.pushState('', '', '/case-studies/?case=' + $(this).attr('case'));
					}
				});
			}			
		})();
		...
		(function(){
			...
			function setSidebarHeight(eventType){
				$(".portfolio-sidebar .menu:visible").each(function(){
					if(eventType=='scroll'){
						if($(this).find("li.selected").closest(".sector").hasClass("open")){
	...
	function initGoogleMap(){
		...
		*/
		...
		function loadGoogleMap(){
			$(".bottom .google-map").each(function(){
				var mapCenter = new google.maps.LatLng($(this).attr('data-lat'), $(this).attr('data-lng'));
				var markerPos = new google.maps.LatLng($(this).attr('data-lat'), $(this).attr('data-lng'));
				var mapOptions = {
						center: mapCenter,
						zoom: 15,			
						panControl: false,
				...
				var linkM=$(this).attr('data-marker-link');
		...
		function setGoogleMap(){
			$('#contact_map_ukraine').gmap3({
				...
				marker: {
					...
					options: {
						icon: 'https://'+document.domain+'/ico-map-2.png'
					},
					events: {
						click: function() {
							window.open("https://www.google.com/maps/place/вулиця+Ярославська,+56а,+Київ,+Украина/@50.4714104,30.5153209,17z");
				...
				overlay: {
					...
					options:{
						content: '<div class="map-info-window">' +
									...
									'</p>' +
									'<p class="phone">' +
									'	<a href="tel:13213288379">1 321 328 8379</a>' +
									'	<br />' +
									'	<a href="tel:37121329191">1 864 558 9180</a>' +
									'</p>' +
									'<p class="email">' +
	...
	// var testExpertiseHeight = $('.expertise-scrollable').height()/2;
	// var testExperienceHeight = $('.experience-scrollable').height()/2;
	// $(window).on("scroll ", function(){
	// 	if(inViewport($('.expertise-scrollable')) > testExpertiseHeight){
			$('.expertise-scrollable').addClass('active');
	// 	} else {
	// 		$('.expertise-scrollable').removeClass('active');
	// 	}
	// 	if(inViewport($('.experience-scrollable')) > testExperienceHeight){
			$('.experience-scrollable').addClass('active');
	// 	} else {
	// 		$('.experience-scrollable').removeClass('active');
	// 	}
	// });
